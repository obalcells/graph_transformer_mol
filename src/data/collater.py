...
def collater_2d(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20):
    items = [
...
def collater_3d(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20):
    items = [
        item for item in items if item is not None and item.x.size(0) <= max_node]
    items = [(item.idx, item.attn_bias, item.attn_edge_type, item.spatial_pos, item.in_degree,
